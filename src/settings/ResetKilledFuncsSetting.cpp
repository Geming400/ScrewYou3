...
class ResetKilledFuncsButtonSetting : public SettingV3 {
public:
    // Once again implement the parse function
    static Result<std::shared_ptr<SettingV3>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
        auto res = std::make_shared<ResetKilledFuncsButtonSetting>();
...
// value!
class ResetKilledFuncsButtonSettingNode : public SettingNodeV3 {
protected:
    ButtonSprite* m_buttonSprite;
    CCMenuItemSpriteExtra* m_button;

    bool init(std::shared_ptr<ResetKilledFuncsButtonSetting> setting, float width) {
        ...
        m_button = CCMenuItemSpriteExtra::create(
            m_buttonSprite, this, menu_selector(ResetKilledFuncsButtonSettingNode::onReset)
        );
    ...
    void onReset(CCObject*) {
        log::info("Resetted killed functions !");
        ScrewYou3Manager::get()->resetKilledFuncs();
    }
...
public:
    static ResetKilledFuncsButtonSettingNode* create(std::shared_ptr<ResetKilledFuncsButtonSetting> setting, float width) {
        auto ret = new ResetKilledFuncsButtonSettingNode();
    ...
    // setting
    std::shared_ptr<ResetKilledFuncsButtonSetting> getSetting() const {
        return std::static_pointer_cast<ResetKilledFuncsButtonSetting>(SettingNodeV3::getSetting());
    }
};

// Create node as before
SettingNodeV3* ResetKilledFuncsButtonSetting::createNode(float width) {
    return ResetKilledFuncsButtonSettingNode::create(
        std::static_pointer_cast<ResetKilledFuncsButtonSetting>(shared_from_this()),
        width
    );
}
...
// Register as before
$execute {
    (void) Mod::get()->registerCustomSettingType("reset-killed-funcs", &ResetKilledFuncsButtonSetting::parse);
}
