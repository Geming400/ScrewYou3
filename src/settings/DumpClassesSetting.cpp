...
class DumpClassesButtonSetting : public SettingV3 {
public:
    // Once again implement the parse function
    static Result<std::shared_ptr<SettingV3>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
        auto res = std::make_shared<DumpClassesButtonSetting>();
...
// value!
class DumpClassesButtonSettingNode : public SettingNodeV3 {
protected:
    ButtonSprite* m_buttonSprite;
    CCMenuItemSpriteExtra* m_button;

    bool init(std::shared_ptr<DumpClassesButtonSetting> setting, float width) {
        if (!SettingNodeV3::init(setting, width))
            return false;
    
        // We just create the button and add it to the setting's menu
        
        m_buttonSprite = ButtonSprite::create("Dump classes", "goldFont.fnt", "GJ_button_01.png", .8f);
        m_buttonSprite->setScale(.5f);
        m_button = CCMenuItemSpriteExtra::create(
            m_buttonSprite, this, menu_selector(DumpClassesButtonSettingNode::onDump)
        );
    ...
    void onDump(CCObject*) {
        matjson::Value asJson = matjson::Value(ScrewYou3Manager::get()->getSurvivingFuncs());
        log::info("Here are the dumped surviving classes for '{}':\n{}", Mod::get()->getName(), asJson.dump());

        FLAlertLayer::create(
            "Dump classes",
            "Surviving classes were dumped. Check your logs !",
            "OK"
        )->show();
    }
...
public:
    static DumpClassesButtonSettingNode* create(std::shared_ptr<DumpClassesButtonSetting> setting, float width) {
        auto ret = new DumpClassesButtonSettingNode();
    ...
    // setting
    std::shared_ptr<DumpClassesButtonSetting> getSetting() const {
        return std::static_pointer_cast<DumpClassesButtonSetting>(SettingNodeV3::getSetting());
    }
};

// Create node as before
SettingNodeV3* DumpClassesButtonSetting::createNode(float width) {
    return DumpClassesButtonSettingNode::create(
        std::static_pointer_cast<DumpClassesButtonSetting>(shared_from_this()),
        width
    );
}
...
// Register as before
$execute {
    (void) Mod::get()->registerCustomSettingType("dump-classes", &DumpClassesButtonSetting::parse);
}
